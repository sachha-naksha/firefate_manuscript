...
% Placeholder for math that was lost when the Word text was pasted in.
% Fill each of these in from the original .docx.
...
\MISSING{} is sc-RNA-seq expression data with \MISSING{} cells and \MISSING{} genes.
\MISSING{} is decomposed into two factors: \MISSING{} and \MISSING{}, with an error term
\MISSING{}. \MISSING{} is the allocation matrix and represents the membership of each
feature to a cellular program. \MISSING{} is the CP matrix and represents a
lower-dimensional representation (\MISSING) of the input data in latent space. Then
using a regression model which applied on CPs, the linear part of the model is generated
...
Here \MISSING{} is the linear part of the model, and \MISSING{} is the set of
significant standalone CPs. Then incorporating the nonlinear relationships, the method
identifies significant interacting CPs:

\begin{equation}
\MISSING
\qquad \text{where} \qquad \MISSING
\end{equation}

Here \MISSING{} and \MISSING{} is the effect size of the interaction term between the
two CPs \MISSING{} and \MISSING{}. \MISSING{} is the set of putative interactions
involving the standalone significant CPs. These interactions arise from the model's
non-linear
architecture, capturing combinatorial regulation and helping improve interpretability
...
and specificity were quantified using the eRegulon score. To ensure comparability across
analyses, we retained only those SCENIC+-derived regulons whose TFs overlapped with
...
identity, we inferred that FIREFate did not specify the cell as belonging to the
predicted fate. This framework produced classification loss scores that quantified the
predictive confidence of rollback analysis, thereby enabling evaluation of the model's
performance in capturing early fate bias.
